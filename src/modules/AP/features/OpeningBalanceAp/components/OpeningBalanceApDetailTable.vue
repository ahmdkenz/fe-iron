  ...
  <div class="ob-detail-table">
    ...
    >
      ...
      <div class="d-flex align-center gap-2 flex-wrap">
        <VBtn
          v-if="outstandingTagihan.length > 0"
          size="small"
          color="success"
          variant="tonal"
        ...
        >
          Muat Data ({{ outstandingTagihan.length }})
        </VBtn>
        <VBtn
          size="small"
          color="primary"
    ...
    >
      ...
      <p class="text-body-2 text-medium-emphasis mb-1">
        Belum ada rincian tagihan asal
      </p>
      <p
        v-if="!readonly"
        class="text-caption text-medium-emphasis"
      >
        Klik "Tambah Tagihan Asal" untuk mendaftarkan tagihan-tagihan vendor asli yang membentuk saldo ini.
      </p>
      <p
        v-else
        class="text-caption text-medium-emphasis"
      >
        Opening balance ini dicatat sebagai lump sum tanpa rincian per tagihan.
      </p>
    </div>
          ...
          <VRow dense>
            ...
            >
              <VCombobox
                ...
                label="No. Invoice/Tagihan Asal"
                density="compact"
                variant="outlined"
                :items="outstandingTagihan"
                item-title="no_tagihan"
                item-value="no_tagihan"
                :return-object="false"
                :loading="loadingOutstanding"
                :no-data-text="loadingOutstanding ? 'Memuat...' : 'Tidak ada tagihan outstanding — ketik manual'"
                placeholder="Pilih atau ketik no. tagihan"
                :rules="[v => !!v || 'No. tagihan wajib diisi']"
                hide-details="auto"
                clearable
              ...
              >
                <template #item="{ props: p, item }">
                  <VListItem
                    v-bind="p"
                    :title="item.raw.no_tagihan"
                  >
                    <template #subtitle>
                      <div class="d-flex gap-3 text-caption mt-1 flex-wrap">
                        <span>{{ formatDate(item.raw.tanggal_tagihan) }}</span>
                        <VChip
                          size="x-small"
        ...
        <div class="ob-items-section">
          <div class="ob-items-section__header">
            <div class="d-flex align-center gap-2">
              <VIcon
                ...
                class="text-medium-emphasis"
              />
              <span class="text-caption font-weight-semibold text-uppercase text-medium-emphasis">Item Tagihan</span>
              <VChip
                size="x-small"
                color="secondary"
                variant="tonal"
          ...
          >
            <span class="text-caption text-medium-emphasis">
              Belum ada item. Klik "Tambah Item" untuk mendetailkan barang/jasa dalam tagihan ini.
            </span>
          </div>
    ...
    <template v-else>
      ...
      >
        <div class="ob-readonly-row__header">
          <div class="d-flex align-center gap-2 flex-wrap">
            <span class="text-caption text-medium-emphasis font-weight-semibold text-uppercase">#{{ i + 1 }}</span>
            <VChip
              size="small"
              color="secondary"
              variant="tonal"
    ...
    <OutstandingTagihanApPickerDialog
      v-model="pickerOpen"
      :tagihan-list="outstandingTagihan"
      :loading="loadingOutstanding"
...
import OpeningBalanceApDetailItemRow from './OpeningBalanceApDetailItemRow.vue'
import OutstandingTagihanApPickerDialog from './OutstandingTagihanApPickerDialog.vue'
...
function mapInvoiceItems(tagihanItems) {
  if (!tagihanItems?.length) return []

  return tagihanItems.map(item => ({
    ...createItem(),
...
function handleInvoiceSelect(rowIndex, noTagihan) {
  const t = props.outstandingTagihan.find(x => x.no_tagihan === noTagihan)
  if (!t) { emitRows() 
  ...
  row.no_invoice_asal      = t.no_tagihan
  row.tanggal_invoice_asal = t.tanggal_tagihan ?? ''
  row.jumlah_tagihan_asal  = t.total_tagihan
  row.sisa_tagihan_asal    = t.sisa_tagihan
  row.deskripsi            = `Sisa tagihan ${t.no_tagihan}`
  row.keterangan           = t.keterangan ?? ''
  row.items                = mapInvoiceItems(t.items)
  emitRows()
}

function loadSelectedOutstanding(selected) {
  if (!selected?.length) return
  rows.value = selected.map(t => ({
    ...createRow(),
    no_invoice_asal: t.no_tagihan,
    tanggal_invoice_asal: t.tanggal_tagihan ?? '',
    deskripsi: `Sisa tagihan ${t.no_tagihan}`,
    jumlah_tagihan_asal: t.total_tagihan,
    sisa_tagihan_asal: t.sisa_tagihan,
    keterangan: t.keterangan ?? '',
    items: mapInvoiceItems(t.items),
  }))
  expandedRows.value = new Set()
  emitRows()
}
